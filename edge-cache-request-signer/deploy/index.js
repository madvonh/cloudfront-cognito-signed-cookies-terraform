...
const setAuthCacheValues = async () => {
  if (cache.privateKeyRef == null) {
    cache.privateKeyRef = await loadParameter(`devsignauth-signing-key-ref`);
  }

  if (cache.cloudfrontKeypair == null) {
    cache.cloudfrontKeypair = await loadParameter(
      `devsignauth-cloudfront-keypair-id`
    );
  }

  if (cache.region == null) {
    cache.region = await loadParameter(`devsignauth-region`);
  }

  if (cache.userPoolId == null) {
    cache.userPoolId = await loadParameter(`devsignauth-user-pool-id`);
  }

  if (cache.clientId == null) {
    cache.clientId = await loadParameter(`devsignauth-client-id`);
  }
